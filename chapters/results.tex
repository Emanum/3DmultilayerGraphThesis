...
Therefore, we decided to test our application in a performance evaluation in Section \ref{sec:performanceEvaluation} and gather different user feedback in the form of a heuristic evaluation in Section \ref{sec:heuristicEvaluation}.
...
Therefore, we assume a lower performance during the initial layout calculation is not as critical as the permanent exploration performance. 
...
A big performance issues might be the calculation of the intersected objects from ray casting by the virtual laser pointer. This could be improved by taking advantage of the hierarchical layout. A ray intersection of child nodes from different parents than the one the user is currently inside is not possible. Therefore, these nodes could be excluded from the ray intersection checks. 
...
As the application runs in the browser, we set up a website \cite{thesisWebsite} with an included introduction to the topic, instructions on how to start and interact with the visualization and multiple datasets with various sizes with the application itself. Afterwards we invited users to participate in the evaluation and fill out an online form we prepared beforehand.
\\
Besides doing an online evaluation we also had one participant that tried the visualization on site. During and afterwards the exploration we conducted an interview.
