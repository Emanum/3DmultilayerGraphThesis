...
\begin{figure}[h]
    ...
    \caption[Distribution of all simulation steps.]{Distribution of all simulation steps for a dataset with 4 hierarchical layers. Each layer receives the same amount of simulation steps, in this case 25 from 100.} % Remove the [...] argument if the original caption should be used in the figure list.
...
%Our initial motivation was to build an immersive visualization for hierarchical networks, that enable users to explore even bigger networks than with classical 2D approaches.
%With the defined goals in Section \ref{chap:ps-requirements} we described the system we wanted to create.
%In the following sections the techniques which fulfill these requirements are described.\\
%All these different methods we apply allows us in the end to provide a first visualization for hierarchical networks in VR.
